...
namespace Tao_OpenGL_Initialization_Test
{
    ...
    { 
                ...
                if(key <= N*N) {
                    ...
                    for (int i = 1; i <= 40; i++)
                    {
                        for (int j = 1; j <= 40; j++)
                        {
                            ...
                            List<int> l = get_nums(computingX(x_i, y_i, 1), computingY(x_i, y_i, 1), computingZ(x_i, y_i, 1));
                            if (l != null)
                            {
                                hash.Add(l[0]);
                            }
        ...
        private List<int> get_nums(double x, double y, double z)
        {
            double max;
            ...
            if (z == 1){
                index.Add((int)((y_0 - y) / d) * N + (int)((x - x_0) / d) + 1);
            } else if (y == 1)
            {
                index.Add((int)((z_0 - z) / d) * N + (int)((x - x_0) / d) + 1 + N*N);
            }
            else
            {
                index.Add((int)((z_0 - z) / d) * N + (int)((y + y_0) / d) + 1 + 2 * N * N);
            }

            if (!graph.ContainsKey(index[0]))
            {
                return null;
            }
